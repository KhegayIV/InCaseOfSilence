/*:
 * @plugindesc Allows to store notetags in text files and reference them from database objects and events. Modified by L'James to allow parameters
 * <Iavra Note Files>
 * @author Iavra
 *
 * @param File Path
 ...
 * 
 * @param Param Prefix
 * @desc Prefix of parameter in noteaf
 * @default $$
 *
 * @help
 ...
 * }
 * 
 * Notes can contain $$1, $$2 etc. - these strings will be replaced by parameters
 * 
...
(function() {
	...
	var _paramPrefix = _params['Param Prefix'];
    ...
    var replace = function(note) {
        if(_regex.test(note)) {
            return replace(note.replace(_regex, function(match, key, args) { 
            	return (args || "").split(/\s*,,\s*/).reduce(function(d, arg, index){
                		return d.split(_paramPrefix+(index + 1)).join(arg);
					}, _data[key] || '');
			}));
        }
        return note;
